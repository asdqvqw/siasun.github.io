<template>
    <el-button type="primary" @click="addCard" style="margin-left: 1vw;" >添加卡片</el-button>
               <div style="display: flex; gap: 20px; flex-wrap: wrap; margin-top: 2%;">
                   <el-card v-for="(item, index) in jsondata.Emg" :key="index" class="button-card"
     ...
     const removeCard = (index) => {
       jsondata.value.Emg.splice(index, 1);
     };
     
     // 处理类型变化
     const handleTypeChange = (index) => {
       if (![1, 2, 4].includes(jsondata.value.Emg[index].io_input[0])) {
           jsondata.value.Emg[index].io_input[1] = 2; // 重置路数
       }
       if (![3].includes(jsondata.value.Emg[index].io_input[0])) {
           jsondata.value.Emg[index].io_input[2] = 2;
           jsondata.value.Emg[index].io_input[3] = 2;
       }
     };
     
     // 判断是否显示路数输入框
     const showRouteInput = (index) => {
       return [1, 2, 4].includes(jsondata.value.Emg[index].io_input[0]);
     };
     ...
     // 检查是否为GPIO
     const isGPIO = (index) => {
       return jsondata.value.Emg[index].io_input[0] === 5; // GPIO 的值
     };
     
     // 检查是否为本地IO
     const isLocalIO = (index) => {
       return jsondata.value.Emg[index].io_input[0] === 3; // 本地IO 的值
     };
     
     // 格式化 bit5
     const formatbit5 = (index) => {
       const decimal = parseInt(jsondata.value.Emg[index].io_input[3], 16);
       jsondata.value.Emg[index].io_input[3] = isNaN(decimal) ? 2 : parseInt(jsondata.value.Emg[index].io_input[3], 16);
     };
     </script>
