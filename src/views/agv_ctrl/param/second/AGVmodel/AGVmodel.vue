<template>
    <div>
        <div>
            <el-checkbox v-model="RealyTimeisChecked" @change="RealyTime">轮或舵因伺服响应慢需要设置延时时间</el-checkbox><br>
            使能前延时时间(ms):<el-input v-model="RealyTimeinput1" :disabled="!RealyTimeisChecked" type="number" style="width: 10%;" @change="RealyTime"></el-input>
            给定前延时时间(ms):<el-input v-model="RealyTimeinput2" :disabled="!RealyTimeisChecked" type="number" style="width: 10%;" @change="RealyTime"></el-input>
        </div>
        <br>
        <div>
...
const RealyTime = () =>{
    ...
    if(!RealyTimeisChecked.value)
    {
        jsondata.value.AGVmodel.RealyTime =[];
    }else{
        jsondata.value.AGVmodel.RealyTime =[parseInt(RealyTimeinput1.value),parseInt(RealyTimeinput2.value)];
    }
...
const handleChange = () => {
    switch (selected.value) {
        case '1':
            ...
            jsondata.value.AGVmodel.type = 1;
            return;
        case '2':
            ...
            selected4.value = false;
            selected5.value = false;
            jsondata.value.AGVmodel.type = 2;
            return;
        case '3':
            selected1.value = false;
            selected2.value = false;
            ...
            selected4.value = false;
            selected5.value = false;
            jsondata.value.AGVmodel.type = 3;
            return;
        case '4':
            selected1.value = false;
            selected2.value = false;
            ...
            selected5.value = false;
            jsondata.value.AGVmodel.type = 4;
            return;
        case '5':
            selected1.value = false;
            selected2.value = false;
            ...
            jsondata.value.AGVmodel.type = 5;
            return;
    }
