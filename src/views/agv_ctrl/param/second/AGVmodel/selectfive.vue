        ...
        <el-dialog :title="title" v-model="dialogVisible" :visible="dialogVisible" width="900px"
            ...
            <el-form ref="form" :model="newRow" label-width="200px">
                ...
                <el-form-item label="车体坐标系坐标X(ms):">
                    <el-input v-model.number="newRow.AGVposX" type="number" @change="AGVpos"></el-input>
                </el-form-item>
                <el-form-item label="车体坐标系坐标Y(ms):">
                    <el-input v-model.number="newRow.AGVposY" type="number" @change="AGVpos"></el-input>
                </el-form-item>
            </el-form><br>
            <h3>驱动类型:</h3>
...
const addNewRow = () => {
    newRow.value.name = '';
    newRow.value.AGVposX = 0;
    newRow.value.AGVposY = 0;
...
const handleAddRow = () => {
    if (newRow.value.name) {
        if (newRow.value.editingIndex !== -1) {
            ...
            // 新增模式下添加数据
            if (newRow.value.diver.morehot.value !== undefined) {
                newRow.value.diver.morehot = newRow.value.diver.morehot.value;
            }

            if (newRow.value.steel.sevorestu.value !== undefined) {
                newRow.value.steel.sevorestu = newRow.value.steel.sevorestu.value;
            }

            if (newRow.value.steel.sevoreable.value !== undefined) {
                newRow.value.steel.sevoreable = newRow.value.steel.sevoreable.value;
            }

            if (newRow.value.steel.LimitOne.value !== undefined) {
                newRow.value.steel.LimitOne = newRow.value.steel.LimitOne.value;
            }

            if (newRow.value.steel.LimitTwo.value !== undefined) {
                newRow.value.steel.LimitTwo = newRow.value.steel.LimitTwo.value;
            }

            if (newRow.value.steel.LimitZero.value !== undefined) {
                newRow.value.steel.LimitZero = newRow.value.steel.LimitZero.value;
            }
