<template>
    <el-button type="primary" @click="addCard" style="margin-left: 1vw;" >添加卡片</el-button>
               <div style="display: flex; gap: 20px; flex-wrap: wrap; margin-top: 2%;">
                   <el-card v-for="(item, index) in jsondata.SafetyLoop.Emg" :key="index" class="button-card"
     ...
     // 添加卡片
     const addCard = () => {
       jsondata.value.SafetyLoop.Emg.push({
     ...
     const removeCard = (index) => {
       jsondata.value.SafetyLoop.Emg.splice(index, 1);
     };
     
     // 处理类型变化
     const handleTypeChange = (index) => {
       if (![3, 2, 5].includes(jsondata.value.SafetyLoop.Emg[index].io_input[0])) {
           jsondata.value.SafetyLoop.Emg[index].io_input[1] = 2; // 重置路数
       }
       if (![1].includes(jsondata.value.SafetyLoop.Emg[index].io_input[0])) {
           jsondata.value.SafetyLoop.Emg[index].io_input[2] = 2;
           jsondata.value.SafetyLoop.Emg[index].io_input[3] = 2;
       }
     };
     
     // 判断是否显示路数输入框
     const showRouteInput = (index) => {
       return [3, 2, 5].includes(jsondata.value.SafetyLoop.Emg[index].io_input[0]);
     };
     ...
     // 检查是否为GPIO
     const isGPIO = (index) => {
       return jsondata.value.SafetyLoop.Emg[index].io_input[0] === 4; // GPIO 的值
     };
     
     // 检查是否为本地IO
     const isLocalIO = (index) => {
       return jsondata.value.SafetyLoop.Emg[index].io_input[0] === 1; // 本地IO 的值
     };
     
     // 格式化 bit5
     const formatbit5 = (index) => {
       const decimal = parseInt(jsondata.value.SafetyLoop.Emg[index].io_input[3], 16);
       jsondata.value.SafetyLoop.Emg[index].io_input[3] = isNaN(decimal) ? 2 : parseInt(jsondata.value.SafetyLoop.Emg[index].io_input[3], 16);
     };
     </script>
