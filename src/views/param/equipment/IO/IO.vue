...
const generateJSON = () => {
    if (selected.value === "2" || selected.value === "3" || selected.value === "5") {
        newRow.value.config = [parseInt(selected.value), parseInt(selectedcan.value), parseInt(selectedcan1.value), parseInt(selectedcan2.value), parseInt(selectedcan3.value)]
    } else if (selected.value === "1") {
        newRow.value.config = [parseInt(selected.value),0, parseInt(selectedcan.value), parseInt(selectedcan1.value), parseInt(selectedcan2.value)]
    } else if (selected.value === "4") {
        const decimal = parseInt(selectedcan.value, 16);
        decimalOutput.value = isNaN(decimal) ? '' : decimal.toString();
        newRow.value.config = [parseInt(selected.value),0,0, parseInt(decimalOutput.value), parseInt(selectedcan1.value)]
    }
    console.log('generateJSON', newRow.value.config)
};
...
const putJSON = () => {
    ...
    if (newRow.value.config.value !== undefined) {
        if (newRow.value.config.value[0] === 2 || newRow.value.config.value[0] === 3 || newRow.value.config.value[0] === 5) {
            selected.value = newRow.value.config.value[0].toString();

            selectedcan.value = newRow.value.config.value[1].toString();
            selectedcan1.value = newRow.value.config.value[2].toString();
            selectedcan2.value = newRow.value.config.value[3].toString();
            selectedcan3.value = newRow.value.config.value[4].toString();

            handleChange();
        } else if (newRow.value.config.value[0] === 1) {

            selected.value = newRow.value.config.value[0].toString();
            // selectedcan.value = '0';
            selectedcan.value = newRow.value.config.value[2].toString();
            selectedcan1.value = newRow.value.config.value[3].toString();
            selectedcan2.value = newRow.value.config.value[4].toString();

            handleChange();
        } else if (newRow.value.config.value[0] === 4) {
            selected.value = newRow.value.config.value[0].toString();
            const decimal = parseInt(newRow.value.config.value[3], 10);
            hexOutput.value = isNaN(decimal) ? '' : decimal.toString(16);
            // selectedcan.value = '0';
            // // selectedcan.value = newRow.value.config.value[1].toString();
            // selectedcan1.value = '0';
            selectedcan.value = hexOutput.value.toString();
            selectedcan1.value = newRow.value.config.value[4].toString();
            handleChange();
        }
        ...
        if (newRow.value.config !== undefined) {
            if (newRow.value.config[0] === 2 || newRow.value.config[0] === 3 || newRow.value.config[0] === 5) {
                selected.value = newRow.value.config[0].toString();

                selectedcan.value = newRow.value.config[1].toString();
                selectedcan1.value = newRow.value.config[2].toString();
                selectedcan2.value = newRow.value.config[3].toString();
                selectedcan3.value = newRow.value.config[4].toString();

                handleChange();
            } else if (newRow.value.config[0] === 1) {
                ...
                handleChange();
            } else if (newRow.value.config[0] === 4) {
                selected.value = newRow.value.config[0].toString();
                const decimal = parseInt(newRow.value.config[3], 10);
                hexOutput.value = isNaN(decimal) ? '' : decimal.toString(16);
